...
{
  $reps = cdmaGetRepeatEntryList($starttime,
...
foreach ( $rooms as $room_id )
{
  ...
  {
    $tmp = cdmaCheckFree($room_id, $starttime, $endtime-1, $ignore_id, 0);
    if (!empty($tmp))
      {
        $valid_booking = FALSE;
        $conflicts .= $tmp;
...
if ($valid_booking)
{
  foreach ( $rooms as $room_id )
  {
    ...
    if ($edit_type == "series")
    {
      $booking = cdmaCreateRepeatingEntrys($starttime,
                                           $endtime,
                                           $rep_type,
                                           $rep_enddate,
                                           $rep_opt,
          ...
          $mail_settings['book_admin_on_provisional'])
      {
        ...
        // and if cdmaCreateRepeatingEntrys is successful
    ...
    else
    {
      ...
      // Create the entry:
      $new_id = cdmaCreateSingleEntry($starttime,
                                      $endtime,
                                      $entry_type,
                                      $repeat_id,
                                      $room_id,
  ...
  if (isset($id))
  {
    cdmaDelEntry($user, $id, ($edit_type == "series"), 1);
  }
