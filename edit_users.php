...
$first_name = get_form_var('first_name', 'string');
$last_name = get_form_var('last_name', 'string');
		...
		foreach ($fields as $field)
		{
			...
			switch($fieldname)
			{
				...
				switch($fieldname)
				{
					...
					break;
					case 'phone';
					if (!empty($invlide_phone))
					{
						echo "<p class=\"error\">" . get_vocab("invalid_phone") . "</p>\n";
					}
				}
	...
	else
	{
		...
		foreach ($fields as $field)
		{
			...
			// re-enter it.  (Instead of passing the data in a query string we
				// could pass them as session variables, but at the moment CDMA does
...
/* Print the standard CDMA header */
